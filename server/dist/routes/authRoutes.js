...
authRouter.use("/admin-login-auth", authMiddleware_1.authenticate, authMiddleware_1.authorize_admin, (req, resp) => {
    resp.status(200).json({ message: "Admin role successfully authorized" });
    return;
});
authRouter.use("/user-login-auth", authMiddleware_1.authenticate, authMiddleware_1.authorize_user, (req, resp) => {
