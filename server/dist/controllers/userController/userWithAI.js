...
const userWithAI = async (req, resp, userMessage) => {
  ...
  try {
    ...
    const intentPromptPath = path_1.default.join(
      process.cwd(),
      "src",
      "controllers",
      "userController",
    ...
    const queryPromptPath = path_1.default.join(
      process.cwd(),
      "src",
      "controllers",
      "userController",
    ...
    const finalPromptPath = path_1.default.join(
      process.cwd(),
      "src",
      "controllers",
      "userController",
    ...
    const intentResponse = await (0, sendToOpenChat_1.sendToOpenChat)(
      intentPrompt
    );
    ...
    if (intentResponse.message.trim() === "database_query") {
      const timelinePromptPath = path_1.default.join(
        process.cwd(),
        "src",
        "controllers",
        "userController",
      ...
      const timelinePrompt = fs_1.default
        ...
        .replace("[[message]]", userMessage);
      const timelineResponse = await (0, sendToOpenChat_1.sendToOpenChat)(
        timelinePrompt
      );
      ...
      if (timelineResponseMessage === "newFollow_up") {
        const queryPrompt = fs_1.default
          ...
          .replace("[[message]]", userMessage);
        const queryResponse = await (0, sendToOpenChat_1.sendToOpenChat)(
          queryPrompt
        );
        ...
        if (!queryResult || queryResult.length === 0) {
          ...
          const queryFailedResponse = await (0,
          sendToOpenChat_1.sendToOpenChat)(queryFailedPrompt);
        ...
        const finalResponse = await (0, sendToOpenChat_1.sendToOpenChat)(
          finalPrompt
        );
      ...
      if (timelineResponseMessage === "follow_up") {
        const followUpPromptPath = path_1.default.join(
          process.cwd(),
          "src",
          "controllers",
          "userController",
        ...
        const followUpPromptResponse = await (0,
        sendToOpenChat_1.sendToOpenChat)(followUpPrompt);
    ...
    if (intentResponse.message.trim() === "self_question") {
      ...
      const selfQuestionPromptPath = path_1.default.join(
        process.cwd(),
        "src",
        "controllers",
        "userController",
      ...
      const selfQuestionPromptResponse = await (0,
      sendToOpenChat_1.sendToOpenChat)(selfQuestionPrompt);
      const insertedAIMessage = await messages.insertOne({
        user_id: userID,
        sender: "ai",
    ...
    if (intentResponse.message.trim() === "general_question") {
      const nonFollowUpPromptPath = path_1.default.join(
        process.cwd(),
        "src",
        "controllers",
        "userController",
      ...
      const nonFollowUpPrompt = fs_1.default
        ...
        .replace("[[message]]", userMessage);
      const nonFollowUpResponse = await (0, sendToOpenChat_1.sendToOpenChat)(
        nonFollowUpPrompt
      );
